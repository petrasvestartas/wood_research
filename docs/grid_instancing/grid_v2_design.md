...
## As built 2026-09-25 (lean pass)

The template at wood `451299f` had grown to 3,466 core lines and 24 grid examples; petras asked for the bare minimum with the most flexibility. `grid_lean_critique.md` lists what was cut. What stands now, and where it departs from the sections below:

- **Files.** `grid.h` (public: `Pattern`, `Profiles`, `Framing`, `Level`, `Building`, `compute_bays`), `grid_plan.h` (internal: the plan geometry declarations and the `Context` / `Member` / `End` / `Station` records with the joint rules), `grid_plan.cpp`, `grid_levels.cpp`, `grid_joints.cpp`, `grid.cpp`. No `clash.h`, no `grid_joints.h`, no `to_mesh`: a BRep goes in as `BRep::mesh()`.
- **Public API.** `Profiles` has five roles (column, girder, beam, purlin, brace); roles 4 / 5 (edge girder, edge beam) and `Framing.edge` are gone: a perimeter member is the girder, beam or purlin its line would carry, all perimeter members share one rank, so an L corner mitres and an interior member butts into the perimeter line. `Level` is `{z, cores, plan}` (`rings` dropped); `Building` is `{levels, braces, tolerance}` (`pattern` dropped: purlin stations are spaced over the bay, not the unclipped cell). One `from_solid(Mesh)`.
- **Attributes.** Face `system`, `span`, `floor`; edge `role`, `family`, `boundary`, `wall`, `line`; vertex `column`, `boundary`, `line_a`, `line_b`. The per-member `width` / `depth` / `drop`, per-face `spacing` / `thickness` / `core` and vertex `through` overrides are gone.
- **Cores stay out of the arrangement.** A core ring makes pinwheel walls, a deck hole (one Clipper difference with the outer wall ring), `column` 0 inside and within the wall band, and every member and station crossing it is split by `compute_pieces` and cut at the wall's outer faces. The core-ring weld order, `is_core_edge`, the core face attribute and the held-back face holes went with it; a footprint hole must cross a pattern line.
- **Decks are Clipper differences**: the bay pushed out on its perimeter sides, minus the rising column sections (node 2), the core wall rings and the decks built before it. `compute_deck_corner` / `compute_partner`, the sliver merge and the head-loop start rotation are gone.
- **Cuts.** The rule is the union: the column face (nodes 1 and 2), the through member's side or the collinear through member's open end, the bisectors with equal angular neighbours. `is_covered` / `compute_overhang` / `compute_run` (the corner mitre of a member wider than its column) are gone; such a pair still leaves no overlap, the butting end just bears partly in the air.
- **Sections are not resampled**: a curved massing gets its columns where its facet corners are, so the example picks 16 facets for 16 rays.
- **Examples.** Five grid files plus `1_elements_flat` / `1_elements_tree`: `templates_grid_footprint` (B, nine buildings in a row), `templates_grid_solid` (A, six massings), `templates_grid_lines` (C, crea and a braced frame), `templates_grid_reference` (Branch square in three methods, Branch residential, FAST+EPP v1 to v4), `templates_grid_framing` (heads, nodes, purlin drops, seven profiles, and the clash check that fails on any overlap). FAST+EPP v3's missing short-side beams are not reproduced (the override attribute went).
- **Checks.** Every scene: pairwise overlap 0, every element with a contact; `main_all_datasets` dumps byte-identical to `451299f`.
