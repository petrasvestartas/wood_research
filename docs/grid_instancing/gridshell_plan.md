...
- [x] Lamella = curve on surface: a (u, v) polyline traced by RK4 as `Pathfinder.FindPath` (direction by
      largest |dot| with the last 3D step, clip at boundary, stop on no direction, loop tolerance).
- [x] Directions by Euler's formula as `FindNormalCurvature`: cos 2a = (2 kn - k1 - k2) / (k1 - k2),
      kn = 0 for asymptotic curves.
- [x] Seeds are input: uv points, or evenly spaced along a seed curve; no hidden spine.
- [x] Exact per-lamella metrics as `CurveOnSurface`: normal curvature, geodesic curvature, geodesic torsion
      (Schling Eq. 3: kn = k1 cos^2 p + k2 sin^2 p, tg = (k2 - k1)/2 sin 2p).
- [x] Board = the lamella's rectifying developable (Schling Sec. 2.2): rulings r = tg t + kg n in the
      Darboux frame, far edge c(t) + w / (r . e3) r; unrolls exactly straight. Report the ruling angle
      atan(tg / kg) against the normal.
- [x] Stud on the exact surface normal at each crossing; report the fit at the node section (<= 0.1 mm) and
      the twist of the strip against the straight node axis (expected, Schling Sec. 3.4).
- [x] Scenes: cubic saddle (asymptotic and iso), a hyperbolic paraboloid patch.

## Stage 2 - harder NURBS

- [x] Surfaces with varying curvature (skewed saddle, NURBS fit of a minimal surface).
- [x] Several seed layouts; constant normal curvature paths (kn != 0) if cheap.
- [x] Stop at flat points (K -> 0); the first curve free of inflection points.

## Stage 3 - meshes and discrete A-nets (Pottmann)

- [x] Initialise: rotational surfaces (catenoid) from ONE traced asymptotic curve, rotated and reflected
      copies (full 360 degrees); general meshes from traced curves or asymptotic directions + quad mesh.
- [x] Constraints: planar vertex stars n . (v_i - v) = 0, |n|^2 = 1 (Schling Eqs. 6-7).
- [x] Solver: guided projection, regularised Gauss-Newton on vertices and normals jointly (Schling Eq. 14):
...
- [x] Refine: bilinear subdivision of the coarse net, optimise the refined polylines with normals from the
      other polylines ((v- - v+) . n = 0).
- [~] Strips: C3 quintic spline through the nodes, osculating plane tangent to S at the nodes; boards as
      rectifying developables as in stage 1. (done with the cubic through the refined vertices and the shared
      normals; no quintic C3 spline)
- [x] Scenes: relaxed minimal disk, catenoid (rotational A-net), Enneper disk.
