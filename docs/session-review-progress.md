...
`xform`: axis rotation now treats a zero axis as a no-op instead of returning a
cosine-scaled matrix, consistently across C++, Python, and Rust. The 26-test
parity suite passes this regression. Final gates passed C++798/798, Python797/797,
...
`boolean_polyline`: `compute_count` now reports selected output ring sizes for
disjoint polygons, including union and difference, instead of returning zero for
every disjoint operation. The three ports share the regression for intersection,
union, and difference. Final gates passed C++798/798, Python797/797, Rust
debug/release798/798, Rust all-target/all-feature tests and doctests, viewer
wasm/native163+13 ignored, viewer GPU13/13, and viewer doctests. Strict Clippy
...
`convex_hull`: degenerate 3D inputs with no nonzero area and volume now return
their vertices without emitting zero-area faces, consistently across C++, Python,
and Rust. The shared 5-test suite covers the fallback. Final gates passed
C++798/798, Python797/797, Rust debug/release798/798, Rust all-target/all-feature
tests and doctests, viewer wasm/native163+13 ignored, viewer GPU13/13, and viewer
